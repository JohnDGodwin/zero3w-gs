...
#Start PixelPilot
pixelpilot --osd --osd-elements 0 --osd-custom-message --osd-config /config/scripts/osd.json --screen-mode $SCREEN_MODE --dvr-framerate $REC_FPS --dvr-fmp4 --dvr-template $DVR_PATH/record_$DVR_SENTINEL.mp4 &
PID=$!
        ...
        if [ "$AP_MODE" -eq 0 ]; then
            if [ "$DVR_BUTTON_STATE" -eq 1 ]; then
                if [ "$REC_STATE" -eq 0 ]; then
                    ...
                    sed -i "s/^dvr_sentinel = .*/dvr_sentinel = $DVR_SENTINEL/" /config/scripts/dvr_sentinel
                    sleep 1
                fi
